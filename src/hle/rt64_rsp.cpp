...
#       endif
        ...
        for (uint32_t i = dstIndex; i < dstMax; i++) {
            ...
            const hlslpp::float3 ps = (tfPos.xyz / hlslpp::float3(tfPos.w, -tfPos.w, tfPos.w)) * viewport.scale + viewport.translate;
    ...
    uint32_t RSP::appendClippedVertex(uint32_t ga, uint32_t gb, float t) {
        ...
        const interop::RSPViewport &viewport = viewportStack[viewportStackSize - 1];
        const hlslpp::float3 ps = (pt.xyz / hlslpp::float3(pt.w, -pt.w, pt.w)) * viewport.scale + viewport.translate;
        posScreen.emplace_back(ps);
        return newIndex;
    }
